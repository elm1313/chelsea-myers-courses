...
USING (
  ...
  EXISTS (
    ...
    AND (
      -- Allow access to SCORM content and all related files
      name LIKE c.storage_path || '/qu package/scormcontent/%' OR
      name LIKE c.storage_path || '/qu package/%' OR
      name LIKE c.storage_path || '/qu package/scormcontent/lib/%' OR
      name LIKE c.storage_path || '/qu package/scormcontent/rise/%' OR
      name LIKE c.storage_path || '/qu package/scormcontent/sandbox/%' OR
      name LIKE c.storage_path || '/qu package/scormcontent/mondrian/%' OR
      name LIKE c.storage_path || '/qu package/scormcontent/locales/%'
    )
  )
);
